...
MKDoc is an application framework which aims at supporting easy installation and
management of multiple MKDoc products (such as MKDoc::Authenticate, MKDoc::Authorize,
...
sub BEGIN { $::SETUP_CLASS ||= __PACKAGE__ }
...
sub install_httpd_conf
{
    ...
    for (@INC)
    {
	my $from_dir = "$_/MKDoc/Site/httpd_conf";
	-d $from_dir || next;
...
sub install_plugins
{
    ...
    touch ("$SITE_DIR/init/10000_MKDoc::Init::Petal");
    
    # plugins
    touch ("$SITE_DIR/plugin/85000_MKDoc::Plugin::It_Worked");
    touch ("$SITE_DIR/plugin/90000_MKDoc::Plugin::Not_Found");
    print "OK\n";
}
