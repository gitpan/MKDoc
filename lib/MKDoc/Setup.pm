...
MKDoc::Setup - Generic user setup for the MKDoc framework.

=head1 SUMMARY

L<MKDoc::Setup> provides a generic base class to make user-friendly installation
procedures for applications which are meant to integrate in the MKDoc framework.
...
If you want to see actual subclasses, look at L<MKDoc::Setup::MKDoc> and L<MKDoc::Setup::Site>.
...
sub main::install
{
    my $class = $::SETUP_CLASS || 'MKDoc::Setup';
    my $setup = $class->new();
    $setup->process();
}
