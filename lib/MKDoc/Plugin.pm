...
MKDoc::Plugin - Class for atomic MKDoc application functionality


=head1 SUMMARY

The MKDoc plugin system is a fundamental foundation for any application
which uses the MKDoc framework. Plugins are classes which are executed one
after the other in a predefined order.
...
=cut
package MKDoc::Plugin;
use MKDoc::HTTP::Request;
...
sub would_activate
{
    ...
    local *MKDoc::HTTP::Response::path_info;
    *MKDoc::HTTP::Response::path_info = sub { $path_info };
...
sub language
{
    ...
    return new MKDoc::Language ($lang);
}
...
Returns the MKDoc::HTTP::Request object singleton.

=cut
sub request
{
    return MKDoc::HTTP::Request->instance();
}
...
Returns the MKDoc::HTTP::Response object singleton.

=cut
sub response
{
    return MKDoc::HTTP::Response->instance();
}
